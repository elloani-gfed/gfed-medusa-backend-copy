...
- **Render lifecycle automation** (`.github/workflows/manage-render-prod-service.yml`): Manual `workflow_dispatch` with inputs for `action` (`suspend`/`resume`), `start_datetime`, `end_datetime`, and `timezone`. The workflow waits until the start time, applies the requested action only if needed, then reverts the service to its original state at end time. If the service is already in the requested state at start, it skips apply/revert. For reliability on GitHub-hosted runners, schedule windows are limited to roughly 5h 50m from dispatch time.
